...
class IndexController extends BaseController{
    ...
    // 生成随机账号
    public function pawd(){
           $pwd = '';
           $length = 6;
            $pattern = '1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLOMNOPQRSTUVWXYZ';
            for($i = 0; $i < $length; $i ++) {
                $pwd .= $pattern {mt_rand ( 0, 61 )}; //生成php随机数
            }
            $where['onepwd'] = $pwd;
            ...
            $db = M('qy_qyzh');
            $con = $db->add($where);
            $accout = date("Ymd")."00".$con;
            $cons = $db->where(array('id'=>$con))->save(array('accout'=>$accout));
            if($cons){
                $this->redirect('index/accout',array('id'=>$con));
            }
    }
    // 园区账号管理
    public function accout(){
        $db = M('users');
        $accout = M('qy_qyzh');
        $this->conss = $accout->where(array('id'=>I('get.id')))->find();
